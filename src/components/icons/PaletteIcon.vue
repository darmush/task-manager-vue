<template>
      <svg class="stroke-gray-text-light stroke-icon-width h-5 w-5"
           viewBox="0 0 22 22"
           fill="none"
           xmlns="http://www.w3.org/2000/svg">
            <path
                  d="M12.5 6C12.7761 6 13 5.77614 13 5.5C13 5.22386 12.7761 5 12.5 5C12.2239 5 12 5.22386 12 5.5C12 5.77614 12.2239 6 12.5 6Z" />
            <path
                  d="M16.5 10C16.7761 10 17 9.77614 17 9.5C17 9.22386 16.7761 9 16.5 9C16.2239 9 16 9.22386 16 9.5C16 9.77614 16.2239 10 16.5 10Z" />
            <path
                  d="M7.5 7C7.77614 7 8 6.77614 8 6.5C8 6.22386 7.77614 6 7.5 6C7.22386 6 7 6.22386 7 6.5C7 6.77614 7.22386 7 7.5 7Z" />
            <path
                  d="M5.5 12C5.77614 12 6 11.7761 6 11.5C6 11.2239 5.77614 11 5.5 11C5.22386 11 5 11.2239 5 11.5C5 11.7761 5.22386 12 5.5 12Z" />
            <path d="M12.5 6C12.7761 6 13 5.77614 13 5.5C13 5.22386 12.7761 5 12.5 5C12.2239 5 12 5.22386 12 5.5C12 5.77614 12.2239 6 12.5 6Z"
                  stroke-linecap="round"
                  stroke-linejoin="round" />
            <path d="M16.5 10C16.7761 10 17 9.77614 17 9.5C17 9.22386 16.7761 9 16.5 9C16.2239 9 16 9.22386 16 9.5C16 9.77614 16.2239 10 16.5 10Z"
                  stroke-linecap="round"
                  stroke-linejoin="round" />
            <path d="M7.5 7C7.77614 7 8 6.77614 8 6.5C8 6.22386 7.77614 6 7.5 6C7.22386 6 7 6.22386 7 6.5C7 6.77614 7.22386 7 7.5 7Z"
                  stroke-linecap="round"
                  stroke-linejoin="round" />
            <path d="M5.5 12C5.77614 12 6 11.7761 6 11.5C6 11.2239 5.77614 11 5.5 11C5.22386 11 5 11.2239 5 11.5C5 11.7761 5.22386 12 5.5 12Z"
                  stroke-linecap="round"
                  stroke-linejoin="round" />
            <path d="M11 1C5.5 1 1 5.5 1 11C1 16.5 5.5 21 11 21C11.926 21 12.648 20.254 12.648 19.312C12.648 18.875 12.468 18.477 12.211 18.187C11.921 17.898 11.773 17.535 11.773 17.062C11.7692 16.8419 11.8098 16.6233 11.8922 16.4192C11.9747 16.2151 12.0975 16.0298 12.2531 15.8741C12.4088 15.7185 12.5941 15.5957 12.7982 15.5132C13.0023 15.4308 13.2209 15.3902 13.441 15.394H15.437C18.488 15.394 20.992 12.891 20.992 9.84C20.965 5.012 16.461 1 11 1Z"
                  stroke-linecap="round"
                  stroke-linejoin="round" />
      </svg>
</template>
